...
class ProductAttributeSyncJob implements ShouldQueue
{
    ...
    {
        Log::info("Processing product attribute sync batch", [
        ...
        try {
            ...
            ])->get($this->apiUrl . 'productattr-sync', [
                'page' => $this->page,
            ...
            // Get attribute option mappings for quick lookups
            $attributeOptionIds = VbStoreAttributeOption::pluck('id', 'bybest_id')->toArray();
            if (empty($attributeOptionIds)) {
                Log::warning("No attribute options found for mapping", ['batch_id' => $this->batchId]);
            }

            // Process product attributes
            $productattrs = $data['data'];
            $totalProcessed = 0;
            $skippedCount = 0;
            ...
            foreach ($productattrs as $item) {
                ...
                try {
                    ...
                    if (!isset($item['id'])) {
                        Log::warning('Product attribute missing ID', ['item' => $item]);
                        $skippedCount++;
                        DB::rollBack();
                        continue;
                    }
                    ...
                    if (!$productId) {
                        Log::warning("Product not found for attribute", [
                    ...
                    $attributeId = isset($attributeOptionIds[$item['atribute_id']]) ? $attributeOptionIds[$item['atribute_id']] : null;
                    if (!$attributeId) {
                    ...
                    DB::rollBack();
                    Log::error("Error processing product attribute", [
                        'batch_id' => $this->batchId,
                        'product_attr_id' => $item['id'] ?? 'unknown',
                        'error' => $e->getMessage(),
            ...
            Log::info("Successfully processed product attributes batch", [
                'batch_id' => $this->batchId,
                'page' => $this->page,
            ...
            Log::error("Failed to process product attributes sync batch", [
                'batch_id' => $this->batchId,
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    private function forceUpdateProgressCounter()
    {
        try {
            ...
            Log::info("Force updated progress counter for product attributes", [
                'batch_id' => $this->batchId,
                'page' => $this->page,
                'success' => $updated
            ]);
